...
class ExtractionPipeline:
    ...
    def run(
        ...
        for i, block in enumerate(blocks):
            ...
            label = f"分析第{block.chapter_start + 1}-{block.chapter_end + 1}章... ({i+1}/{block_count})"
            yield {"stage": "meso", "label": label, "progress_pct": base_pct,
                   "data": {"block_index": i, "chapters": f"{block.chapter_start + 1}-{block.chapter_end + 1}"}}
    ...
    def _run_skeleton(self, chapters, start_pct, end_pct):
        ...
        yield {"stage": "skeleton_done", "label": "骨架层完成",
               "progress_pct": mid,
               "data": {
                   "worldview": sk_result.get("worldview", ""),
                   "plot": sk_result.get("plot", ""),
                   "characters": sk_result.get("characters", ""),
                   "mysteries": sk_result.get("mysteries", ""),
               }}
